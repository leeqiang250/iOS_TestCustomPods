...
- (instancetype)init {
    self = [super init];
    _hashWords = [[NSMutableDictionary alloc] initWithCapacity:[self getOriginalWords].count];
    [self hashSort];
...
- (void)hashSort {
    for (NSString *word in [self getOriginalWords]) [_hashWords setObject:word forKey:[NSString stringWithFormat:@"%lu", word.hash]];
}
...
- (NSString *)getWords:(NSArray *)keys {
    ...
    for (NSString *key in keys) [words addObject:[_wordsSort objectAtIndex:key.integerValue]];
